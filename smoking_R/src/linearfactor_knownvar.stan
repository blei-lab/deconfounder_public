...
model {
    ...
    for(k in 1:K) W1[,k] ~ normal(0, t_alpha[k]);
    for(i in 1:N) to_vector(X_train[i]) ~ normal(to_vector((W0' + Z[i]*W1').*(1-holdout_mask[i])), data_std);
} 
...
generated quantities{
    ...
    for(i in 1:N) {
        ps[i] = normal_lpdf(to_vector(X_all[i]) | to_vector(W0' + Z[i]*W1'), data_std);
        rep_lp[i] = 0;
        vad_lp[i] = 0; 
        for(j in 1:D){
            predmean = W0[j] + Z[i,]*to_vector(W1[j,]);
